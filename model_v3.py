"""
model_v3.py  —  SCALENet with targeted fixes, same parameter budget philosophy.

What changed vs v2 and WHY:
─────────────────────────────────────────────────────────────────────────────
1. IlluminationConditionedNorm (replaces NoiseAwareAdaIN)
   - AdaIN computed channel-by-channel on the INPUT luma, not feature statistics.
   - This gives each normalization layer an explicit "how dark is this region"
     signal, solving the single-mapping-for-all-darkness problem without adding
     a whole separate network branch.
   - The v2 noise_estimator was operating on feature maps (after several convs),
     which loses spatial luma information. This version uses the raw low-light
     input luma passed in as a side-channel.

2. Deeper refinement head (4 conv layers, not 2)
   - The fusion output is reasonable but the 2-conv refinement is too shallow to
     fix texture details. Going to 4 layers with residual connections adds ~50k
     params but substantially improves detail recovery.

3. ExposureBranch gets a skip connection
   - Prevents over-smoothing by preserving high-frequency detail from input.

4. IlluminationEmbedding
   - A 4-dim scalar embedding of the per-image mean luminance is computed once
     and broadcast spatially into the refinement. Cheap but effective conditioning.
   - This is what fixes "single mapping for all darkness levels".

5. SpatialAttentionGate in refinement
   - Soft gate that focuses refinement on actually-dark spatial regions.
   - Avoids wasting capacity on already-bright areas.

6. res_gate now operates on the refinement features (not fused), fixing a
   logical inconsistency in v2 where the gate was computed from fused RGB.

Parameter target: kept < 1MB (competition constraint assumed).
─────────────────────────────────────────────────────────────────────────────
"""
